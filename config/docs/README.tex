...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(}\KeywordTok{defun}\FunctionTok{ rotate-windows }\NormalTok{()}
  \StringTok{"Rotate your windows"}
  \NormalTok{(interactive)}
  \NormalTok{(other-window -}\DecValTok{1}\NormalTok{)}
  \NormalTok{(}\KeywordTok{cond} \NormalTok{((}\KeywordTok{not} \NormalTok{(}\KeywordTok{>} \NormalTok{(count-windows)}\DecValTok{1}\NormalTok{))}
         \NormalTok{(message }\StringTok{"You can't rotate a single window!"}\NormalTok{))}
        \NormalTok{(}\KeywordTok{t}
         \NormalTok{(}\KeywordTok{setq} \NormalTok{i }\DecValTok{1}\NormalTok{)}
         \NormalTok{(}\KeywordTok{setq} \NormalTok{numWindows (count-windows))}
         \NormalTok{(while  (}\KeywordTok{<} \NormalTok{i numWindows)}
           \NormalTok{(}\KeywordTok{let*} \NormalTok{(}
                  \NormalTok{(w1 (}\KeywordTok{elt} \NormalTok{(window-list) i))}
                  \NormalTok{(w2 (}\KeywordTok{elt} \NormalTok{(window-list) (}\KeywordTok{+} \NormalTok{(% i numWindows) }\DecValTok{1}\NormalTok{)))}

                  \NormalTok{(b1 (window-buffer w1))}
                  \NormalTok{(b2 (window-buffer w2))}

                  \NormalTok{(s1 (window-start w1))}
                  \NormalTok{(s2 (window-start w2))}
                  \NormalTok{)}
             \NormalTok{(set-window-buffer w1  b2)}
             \NormalTok{(set-window-buffer w2 b1)}
             \NormalTok{(set-window-start w1 s2)}
             \NormalTok{(set-window-start w2 s1)}
             \NormalTok{(}\KeywordTok{setq} \NormalTok{i (}\KeywordTok{1+} \NormalTok{i)))))))}
\NormalTok{(global-set-key (kbd }\StringTok{"<f6>"}\NormalTok{) 'rotate-windows)}
\end{Highlighting}
\end{Shaded}

Rotaciona os frames no sentido anti-horário. Mantém o foco no frame onde
o usuário está trabalhando.

\subsubsection{Renomeia buffer e
arquivo}\label{renomeia-buffer-e-arquivo}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(}\KeywordTok{defun}\FunctionTok{ rename-current-buffer-file }\NormalTok{()}
  \StringTok{"Renames current buffer and file it is visiting."}
  \NormalTok{(interactive)}
  \NormalTok{(}\KeywordTok{let} \NormalTok{((name (buffer-name))}
        \NormalTok{(filename (buffer-file-name)))}
    \NormalTok{(}\KeywordTok{if} \NormalTok{(}\KeywordTok{not} \NormalTok{(}\KeywordTok{and} \NormalTok{filename (file-exists-p filename)))}
        \NormalTok{(}\KeywordTok{error} \StringTok{"Buffer '%s' is not visiting a file!"} \NormalTok{name)}
      \NormalTok{(}\KeywordTok{let} \NormalTok{((new-name (read-file-name }\StringTok{"New name: "} \NormalTok{filename)))}
        \NormalTok{(}\KeywordTok{if} \NormalTok{(get-buffer new-name)}
            \NormalTok{(}\KeywordTok{error} \StringTok{"A buffer named '%s' already exists!"} \NormalTok{new-name)}
          \NormalTok{(}\KeywordTok{rename-file} \NormalTok{filename new-name }\DecValTok{1}\NormalTok{)}
          \NormalTok{(rename-buffer new-name)}
          \NormalTok{(set-visited-file-name new-name)}
          \NormalTok{(set-buffer-modified-p }\KeywordTok{nil}\NormalTok{)}
          \NormalTok{(message }\StringTok{"File '%s' successfully renamed to '%s'"}
                   \NormalTok{name (file-name-nondirectory new-name)))))))}
\NormalTok{(global-set-key (kbd }\StringTok{"C-x r C-f"}\NormalTok{) 'rename-current-buffer-file)}
\end{Highlighting}
\end{Shaded}

Altera o nome do buffer e do arquivo em disco. Como se o usuário
gravasse o arquivo, renomeasse no disco e abrisse novamente.

\subsubsection{insere linha em branco}\label{insere-linha-em-branco}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(}\KeywordTok{defun}\FunctionTok{ open-line-below }\NormalTok{()}
  \NormalTok{(interactive)}
  \NormalTok{(end-of-line)}
  \NormalTok{(newline)}
  \NormalTok{(indent-for-tab-command))}

\NormalTok{(}\KeywordTok{defun}\FunctionTok{ open-line-above }\NormalTok{()}
  \NormalTok{(interactive)}
  \NormalTok{(beginning-of-line)}
  \NormalTok{(newline)}
  \NormalTok{(forward-line -}\DecValTok{1}\NormalTok{)}
  \NormalTok{(indent-for-tab-command))}
\NormalTok{(global-set-key (kbd }\StringTok{"C-x C-<down>"}\NormalTok{) 'open-line-below)}
\NormalTok{(global-set-key (kbd }\StringTok{"C-x C-<up>"}\NormalTok{) 'open-line-above)}
\end{Highlighting}
\end{Shaded}
